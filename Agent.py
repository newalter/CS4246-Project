...
class QAgent():
    ...
    def get_action(self, observation):
        ...
        if random.random() < self.totalMoves / 1000000:
            return self.max_arg(observation)
        else:
    ...
    def max_lookup(self, observation):
        ...
        maxR = self.Q.get((pos,velocity), -1000)
        maxR = max(maxR, self.Q.get((pos, velocity, 0), -1000))
        maxR = max(maxR, self.Q.get((pos, velocity, 1), -1000))
        maxR = max(maxR, self.Q.get((pos, velocity, 2), -1000))
        return maxR
    ...
    def max_arg(self, observation):
        ...
        velocity = observation[1]
        action = 0
        maxR = self.Q.get((pos, velocity, 0), -1000)
        if maxR < self.Q.get((pos, velocity, 1), -1000):
            maxR = self.Q.get((pos, velocity, 1), -1000)
            action = 1
        if maxR < self.Q.get((pos, velocity, 2), -1000):
            maxR = self.Q.get((pos, velocity, 2), -1000)
            action = 2
        return action
